# Reading Library

**SDA Commons Wiki · South High School**

The Reading Library is a curated collection of texts, primary sources, documents, and literary works that support SDA studio research. Each entry is a structured wiki page with content summary, key passages, standard connections, vocabulary, and studio ideas.

---

## What the Reading Library Is

Every text in this library has been evaluated for use in SDA studios. An entry is more than a citation — it's a fully processed document:

- A plain-language description of what the text is and why it matters
- Connections to specific ELA standards (with standard codes)
- Connections to Social Studies standards (with era or unit specified)
- A key vocabulary list
- A key passage you can use as evidence
- Studio project ideas built around the text

The library is searchable by standard, era/unit, text type, and author. It grows every studio cycle as new texts are processed.

---

## How to Navigate

### By Subject Area

- [Ela](ela) — Literary texts, novels, memoirs, essays, and poetry used in ELA studios
- [Social Studies](social-studies) — Primary sources, historical documents, nonfiction texts organized by era/unit
- [Crosswalk](crosswalk) — Texts that work for both ELA and Social Studies standards simultaneously (the most versatile)

### By Standard

To find texts for a specific ELA standard, check the standard's page — each one links to relevant library entries.
- [R.9](../standards/ela/gr9-10-scales.md#9-10r9) — Reading standard 9 (informational/argumentative analysis)
- [9-10.R.8](../standards/ela/gr9-10-scales.md#9-10r8) — Reading standard 8 (literary elements)
- [R.7](../standards/ela/gr9-10-scales.md#9-10r7) — Style, syntax, word choice

### By Era or Unit

For World History studios:
- [Era 1 — Civilizations](../standards/social-studies/world-history/era-1-civilizations.md) → links to Era 1 texts
- [Era 2 — Middle Ages](../standards/social-studies/world-history/era-2-middle-ages.md) → Era 2 texts
- [Era 3 — Revolutions](../standards/social-studies/world-history/era-3-revolutions.md) → Era 3 texts
- [Era 4 — Global War](../standards/social-studies/world-history/era-4-global-war.md) → Era 4 texts

For US History studios:
- Each unit page ([Unit 1 — Industrial Revolution](../standards/social-studies/us-history/unit-1-industrial.md) through unit-9) links to relevant sources

---

## How to Use a Reading Library Entry

Each entry follows this format:

```
# [Title]
**Author:** [Name]
**Type:** [Primary Source / Literary Text / Historical Document / Nonfiction]
**Source:** [PDF or text link]
**Era/Period:** [Era X or USH Unit X if Social Studies-aligned]

## What This Text Is About
[2 sentences. Plain language. Student-readable.]

## ELA Standards This Connects To
- [R.9](../standards/ela/gr9-10-scales.md#9-10r9) — [why this standard applies]
- [W.4](../standards/ela/writing-standards.md#9-10w4) — [writing connection]

## Social Studies Standards This Connects To
- [WH.6_12.X](/standards/social-studies/world-history/era-X.md#wh6-12x) — [why this connects]

## Key Vocabulary
| Term | Definition |
|------|-----------|
| [word] | [plain-language definition] |

## Key Passage
> [Significant excerpt]
[One sentence on why this passage matters]

## Studio Ideas
1. **[Studio Idea 1]** — EQ: [question] · Standard: [code + era]
2. **[Studio Idea 2]** — EQ: [question] · Standard: [code + era]

## See Also
- [[skinnies/[subject]/[era]/[relevant-skinny]]]
- [[Related Entry](/reading-library/[related-entry.md)]
```

---

## Entry Types

| Type | What It Is | How It's Used in SDA |
|------|-----------|---------------------|
| **Primary Source** | A document created at the time of the event being studied (Hammurabi's Code, the Declaration of Rights of Man, MLK's "Letter from Birmingham Jail") | Used for R.9 analysis, cause-and-effect investigation, perspective comparison |
| **Literary Text** | Fiction, memoir, or poetry that can be analyzed for literary elements | Used for R.8 analysis, W.3/W.4 extended writing, ELA demonstration of learning |
| **Historical Document** | Government documents, legal texts, treaties, speeches (overlaps with primary source) | Used for R.9, source credibility evaluation (IR.4), argumentative writing |
| **Nonfiction** | Articles, essays, books — explanatory or argumentative (can be primary or secondary) | Used for R.4 (main idea tracking), R.9 (argumentative analysis), IR.2 (source interpretation) |

---

## Adding to the Library

New texts can be added to the Reading Library in two ways:

1. **From RAW_INTAKE:** Place a PDF in `~/sda-wiki/RAW_INTAKE/reading_library/` and Hermes will process it into a full library entry.

2. **Project Gutenberg:** Say "add to library: [title]" and Hermes will fetch the plain text and process it directly.

Processed entries are added to the appropriate subfolder and a git commit is made automatically.

---

## All Library Entries

### Social Studies Primary Sources

#### Era 1 — Ancient Civilizations (c. 1750 BCE – 500 CE)

| Text | Author | Date | Standards |
|------|--------|------|-----------|
| [Hammurabi's Code](social-studies/hammurabi-code.md) | Hammurabi | c. 1750 BCE | WH.6_12.2 · WH.6_12.3 · R.9 |
| [The Analects](social-studies/analects-confucius.md) | Confucius | c. 500 BCE | WH.6_12.2 · WH.6_12.3 · R.4 · R.9 |
| [Pericles' Funeral Oration](social-studies/pericles-funeral-oration.md) | Thucydides | 431 BCE | WH.6_12.3 · WH.6_12.4 · WH.6_12.6 · R.9 · W.4 · C.1 |
| [The Melian Dialogue](social-studies/melian-dialogue.md) | Thucydides | 416 BCE | WH.6_12.3 · WH.6_12.5 · WH.6_12.6 · R.9 · W.4 · C.6 |

> **Pair:** Pericles and the Melian Dialogue must be read together — same author, same democracy, 15 years apart.

#### Era 2 — Middle Ages and Early Modern (500–1750 CE)

| Text | Author | Date | Standards |
|------|--------|------|-----------|
| [Magna Carta](social-studies/magna-carta.md) | English barons / King John | 1215 | WH.6_12.2 · WH.6_12.3 · R.9 |
| [Rihla (Travels)](social-studies/ibn-battuta-rihla.md) | Ibn Battuta | 1354 | WH.6_12.1 · WH.6_12.4 · R.9 · IR.4 |
| [The 95 Theses](social-studies/luther-95-theses.md) | Martin Luther | 1517 | WH.6_12.2 · WH.6_12.5 · R.9 · W.4 |
| [A Short Account of the Destruction of the Indies](social-studies/las-casas-short-account.md) | Bartolomé de las Casas | 1542 | WH.6_12.4 · WH.6_12.5 · R.9 · IR.4 · W.4 |

#### Era 3 — Age of Revolutions and Industry (1750–1914)

| Text | Author | Date | Standards |
|------|--------|------|-----------|
| [Common Sense](social-studies/common-sense-paine.md) | Thomas Paine | 1776 | WH.6_12.3 · WH.6_12.5 · R.9 · W.4 |
| [Declaration of the Rights of Man](social-studies/declaration-rights-man.md) | National Assembly of France | 1789 | WH.6_12.3 · WH.6_12.5 · WH.6_12.6 · R.9 |
| [A Vindication of the Rights of Woman](social-studies/wollstonecraft-vindication.md) | Mary Wollstonecraft | 1792 | WH.6_12.3 · WH.6_12.6 · R.9 · W.4 |
| [Haitian Declaration of Independence](social-studies/haitian-declaration-independence.md) | Dessalines / Boisrond-Tonnerre | 1804 | WH.6_12.3 · WH.6_12.5 · WH.6_12.6 · R.9 · W.4 |
| ["The White Man's Burden" + H.T. Johnson response](social-studies/kipling-white-mans-burden.md) | Kipling / Johnson | 1899 | WH.6_12.4 · WH.6_12.5 · WH.6_12.6 · R.7 · R.9 · IR.4 |
| [The Communist Manifesto](social-studies/communist-manifesto.md) | Marx & Engels | 1848 | WH.6_12.2 · WH.6_12.3 · WH.6_12.5 · R.9 · W.4 |
| [Narrative of the Life of Frederick Douglass](social-studies/narrative-frederick-douglass.md) | Frederick Douglass | 1845 | WH.6_12.4 · WH.6_12.5 · US.6_12.2 · US.6_12.4 · R.8 · R.9 · W.4 |
| [The Jungle](social-studies/jungle-sinclair.md) | Upton Sinclair | 1906 | US.6_12.1 · US.6_12.4 · R.8 · W.4 · IR.1–5 |
| ["Dulce et Decorum Est"](social-studies/dulce-et-decorum-est.md) | Wilfred Owen | 1920 | WH.6_12.2 · WH.6_12.5 · R.4 · R.7 · R.8 |

#### Era 4 — Global War and Globalization (1914–Present)

| Text | Author | Date | Standards |
|------|--------|------|-----------|
| [FDR — Four Freedoms Speech](social-studies/fdr-four-freedoms.md) | Franklin D. Roosevelt | 1941 | WH.6_12.2 · WH.6_12.3 · US.6_12.1 · US.6_12.2 · R.9 · W.4 · C.1 |
| [UN Universal Declaration of Human Rights](social-studies/un-declaration-human-rights.md) | United Nations | 1948 | WH.6_12.3 · WH.6_12.6 · US.6_12.1 · US.6_12.4 · R.9 · IR.1–5 |
| [Rivonia Trial Speech](social-studies/mandela-rivonia-speech.md) | Nelson Mandela | 1964 | WH.6_12.3 · WH.6_12.6 · R.9 · W.4 · C.1 |
| [UN Speech — "One Child, One Teacher"](social-studies/malala-un-speech.md) | Malala Yousafzai | 2013 | WH.6_12.2 · WH.6_12.6 · R.9 · W.4 · C.1 |

> **Rights chain:** Hammurabi's Code → Pericles → Magna Carta → Common Sense → Declaration of Rights of Man → Haitian Declaration → FDR Four Freedoms → UN UDHR → Malala

---

### ELA Literary Texts (Dual-Use: also primary sources for history)

| Text | Author | Date | ELA Standards | History Connection |
|------|--------|------|---------------|--------------------|
| [Julius Caesar](ela/julius-caesar-shakespeare.md) | Shakespeare | 1599 | R.8 · R.9 · W.4 | WH Era 1 (Rome) + WH Era 2 (Elizabethan political thought) |
| [Narrative of Frederick Douglass](social-studies/narrative-frederick-douglass.md) | Douglass | 1845 | R.8 · R.9 · W.4 | USH Civil War era primary source |
| [The Jungle](social-studies/jungle-sinclair.md) | Sinclair | 1906 | R.8 · W.4 · IR.1–5 | USH Progressive Era primary source |
| [Fahrenheit 451](ela/fahrenheit-451-bradbury.md) | Bradbury | 1953 | R.8 · R.9 · W.4 | WH Era 4 Cold War cultural anxiety document |
| [Night](ela/night-wiesel.md) | Wiesel | 1960 | R.8 · R.9 · W.4 | WH Era 4 Holocaust memoir / primary source |
| [Warriors Don't Cry](ela/warriors-dont-cry.md) | Beals | 1994 | R.8 · R.9 · W.4 | USH Civil Rights primary source (Little Rock 1957) |
| [Persepolis](ela/persepolis-satrapi.md) | Satrapi | 2000 | R.8 · R.9 · W.4 | WH Era 4 Iranian Revolution primary source |
| [1984](ela/orwell-1984.md) | Orwell | 1949 | R.8 · R.9 · W.4 | WH Era 4 totalitarianism / Cold War |

> **Dual-use note:** All ELA texts above carry both literary standards (R.8 literary analysis) and historical standards as primary sources for their era. See individual entries for full standard crosswalk.

---

*Reading Library index — SDA Commons Wiki · South High School*
*New texts added as they are processed. Check era/unit pages for current entries.*
*See also: [Home](../index.md) · [ELA 9-10 Scales](../standards/ela/gr9-10-scales.md) · [WH Standards Overview](../standards/social-studies/world-history/standards-overview.md)*
